...
Coaches the user through the 31 hands-on labs in `theplatformlab/CKA-Certified-Kubernetes-Administrator/exercises` toward a real CKA pass — sequencing by exam weight, self-grading, and running mock exams — without ever doing the graded work for them. (These exercises are independently-written practice scenarios inspired by CKA domains, not leaked real exam questions — treat them as training, not gospel.)
